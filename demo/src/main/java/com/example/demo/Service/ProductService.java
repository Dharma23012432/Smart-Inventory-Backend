...
public class ProductService {
    ...
    public void updateStock(long id, int soldQuantity) {
        ...
        // Record the sale so we can compute revenue reports
        try {
            saleService.recordSale(product, soldQuantity);
            System.out.println("Recorded sale for " + product.getName() + " qty=" + soldQuantity);
        } catch (Exception ex) {
            System.err.println("Failed to record sale: " + ex.getMessage());
        }
